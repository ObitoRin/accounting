  ...
  setup: (props, context) => {
    ...
    const emojis = computed(() => {
      ...
      return selectedItem.map(category =>
        emojiList.find(item => item[0] === category)?.[1]
          .map(emoji => <li class={emoji === props.modelValue ? s.selectedEmoji : ''}
    ...
    return () => (
      <div class={s.emojiList}>
        <nav>
          {table.map((item, index) => <span class={refSelected.value === index ? s.selected : ''} onClick={() => onClickTab(index)}>{item[0]}</span>)}
        </nav>
        <ol>
          {emojis.value}
        </ol>
